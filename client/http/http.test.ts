...
test("Test simple interpolation and query params", () => {
	let uri = http.buildUriAndParams("/api/update/Page/:id", {
		id: 1,
		queryParam: "test",
	})
	expect(uri).toBe("/api/update/Page/1?queryParam=test")
})
...
test("Test encoding of interpolation and query params", () => {
	...
	expect(uri).toBe("/api/update/Page/id_with%2F_%2F_two_slashes?queryParam=test_%26with%26_two_ampersands")
})
...
test("Dont mutate given JS Object", () => {
	...
	http.buildUriAndParams("/api/update/Page/:id", originalParams)
	if (!originalParams) {
